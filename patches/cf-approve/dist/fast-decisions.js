...
export function checkFastDecision(toolName, toolInput) {
    ...
    // For Bash, also match against the command text — but only for simple
    // commands (no chaining, pipes, redirection, or substitution), so a
    // safe-looking prefix can't smuggle a compound command through.
    const bashCommand = toolName === "Bash" && typeof toolInput.command === "string"
        ? toolInput.command.trim()
        : null;
    const isSimpleCommand = bashCommand !== null && !/[;&|<>`$(){}\n\\]/.test(bashCommand);
    for (const pattern of config.customAllowPatterns) {
        const regex = new RegExp(pattern);
        if (regex.test(toolName) || (isSimpleCommand && regex.test(bashCommand))) {
            return {
                decision: "allow",
