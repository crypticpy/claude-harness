 ...
 * 
 * Provides Claude with tools to search conversation history using GPT-4.1's
 * massive context window as a "trusted advisor" that maintains perfect memory.
 */

import * as fs from 'fs';
import * as path from 'path';

const LOG_DIR = path.join(process.env.HOME!, '.claude', 'hooks', 'unified', 'logs');
...
/**
 * Search conversation history using GPT-4.1
 * 
...
Answer:`;
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(`GPT-4.1 call failed: ${response.status}`);
        }
        ...
        const relevantOps = logs
            .filter(log => {
                const text = `${log.tool_name} ${log.metadata?.file || ''} ${log.output_summary || ''}`.toLowerCase();
...
export async function fileEditHistory(input: {
    ...
    return {
        ...
        recentChanges: (sessionData?.edits || [])
            .slice(-10)
            .map((e: any) => ({
...
function loadLogs(sessionId?: string, lookback: string = 'session'): any[] {
    ...
    if (lookback === 'session' && sessionId) {
        ...
        if (fs.existsSync(sessionLog)) {
            const lines = fs.readFileSync(sessionLog, 'utf-8').split('\n').filter(l => l.trim());
...
function loadMementoConfig(): MementoConfig | null {
    ...
    const hookConfig = path.join(process.env.HOME!, '.claude', 'hooks', 'unified', 'config.json');
...
function getApiKey(): string | null {
    ...
    const permHookConfig = path.join(process.env.HOME!, '.claude-code-fast-permission-hook', 'config.json');
...
export const mementoToolDefinitions = [
    {
        name: 'recall_history',
        description: 'Ask GPT-4.1 about past conversation history. Use this when you need to remember what happened earlier in this session or in previous sessions. The AI advisor has perfect memory of all operations.',
        inputSchema: {
            type: 'object',
            properties: {
                query: {
                    type: 'string',
                    description: 'Question about past operations (e.g. "When did I last edit auth.ts?" or "What changes have I made to the API?")'
                },
                sessionId: {
                    type: 'string',
    ...
    },
    {
        ...
        description: 'Get detailed edit history for a specific file, including how many times it was edited and summaries of recent changes.',
        inputSchema: {
            type: 'object',
            properties: {
                filePath: {
    ...
    },
    {
        ...
        description: 'Search past tool calls with filters. Useful for finding specific operations like "all times I ran tests" or "recent git commands".',
        inputSchema: {
            type: 'object',
            properties: {
                toolName: {
