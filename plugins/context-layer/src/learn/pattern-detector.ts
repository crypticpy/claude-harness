...
export type PatternType =
  | 'debugging-session'      // Same file accessed repeatedly
  | 'refactoring-pattern'    // impact_check followed by edits
  | 'hot-path'               // File accessed across many sessions
  | 'naming-convention'      // Files follow naming pattern
  | 'directory-pattern'      // Directory has consistent structure
  | 'error-prone-file';      // File frequently involved in debugging

interface SessionHistory {
  sessionId: string;
  filesAccessed: Set<string>;
  toolSequence: AccessEvent[];
  startTime: number;
}
 ...
 */
export function startSession(projectPath: string, sessionId: string): void {
 ...
 */
export function endSession(projectPath: string): PatternResult[] {
  const state = getProjectState(projectPath);
 ...
 */
function detectDebuggingPattern(
  filePath: string,
  ...
  const recentAccesses = stats.recentAccesses.filter(
    a => now - a.timestamp < DEBUGGING_WINDOW_MS
  );
 ...
 */
function detectRefactoringPattern(session: SessionHistory): PatternResult[] {
  const patterns: PatternResult[] = [];
...
function detectSessionPatterns(
  ...
  if (sessionDuration < 30 * 60 * 1000) { // Less than 30 minutes
    for (const [filePath, stats] of fileStats) {
      const sessionAccesses = stats.recentAccesses.filter(
        a => a.timestamp >= session.startTime
      );
 ...
 */
function detectNamingConventions(files: Set<string>): PatternResult[] {
  const patterns: PatternResult[] = [];
  const dirPatterns = new Map<string, string[]>();
 ...
 */
function detectDirectoryPatterns(files: Set<string>): PatternResult[] {
  const patterns: PatternResult[] = [];
 ...
 */
export function getDetectedPatterns(projectPath: string): PatternResult[] {
 ...
 */
export function getFileStats(projectPath: string): Map<string, FileAccessStats> {
  const state = projectState.get(projectPath);
 ...
 */
export function clearPatternState(projectPath: string): void {
 ...
 */
export function getHotFilesFromPatterns(projectPath: string): Array<{
  path: string;
  sessionCount: number;
  totalAccesses: number;
  reason: string;
