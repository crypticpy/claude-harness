...
function formatBrainContext(brain: PersistentBrain, projectPath: string): string {
...
function shouldInjectThisSession(projectPath: string, sessionId: string): boolean {
 ...
 */
function checkCompactionRecoveryContext(projectPath: string, sessionId: string): string | null {
...
function detectFromPackageJson(pkg: Record<string, unknown>, stack: StackInfo): void {
  ...
  if (deps['redux'] || deps['@reduxjs/toolkit']) stack.stateManagement = 'Redux';
  ...
  if (deps['playwright'] || deps['@playwright/test']) stack.testing = 'Playwright';
 ...
 */
function detectHybridStack(_projectPath: string, stack: StackInfo): { isHybrid: boolean; details: string } {
  ...
  if (hasPython && hasJS) {
    const pyFramework = stack.frameworks.find(f =>
      ['FastAPI', 'Django', 'Flask'].includes(f)
    ) || 'Python';
    const jsFramework = stack.frameworks.find(f =>
 ...
 */
function determineProjectType(stack: StackInfo, projectPath: string): ProjectType {
...
function identifyKeyFiles(projectPath: string): KeyFile[] {
  ...
  const candidates: Array<{ path: string; purpose: string; importance: KeyFile['importance'] }> = [
