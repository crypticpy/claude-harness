...
function parseFile(content, filePath, options = {}) {
    const opts = { ...types_1.DEFAULT_PARSER_OPTIONS, ...options };
    const ext = path.extname(filePath).toLowerCase();
    if (content.length > opts.maxFileSize) {
        return {
            ...(0, types_1.createEmptyParseResult)('unknown'),
            errors: [`File exceeds max size of ${opts.maxFileSize} bytes`],
        };
    }
    const language = detectLanguage(ext);
    const result = (0, types_1.createEmptyParseResult)(language);
...
function parseTypeScript(content, result, _opts) {
    // Parse imports
...
function parsePython(content, result, _opts) {
    ...
    while (i < lines.length) {
        ...
        if (fromMatch) {
            ...
            for (const name of names) {
                ...
                if (!importName || /^[^a-zA-Z_]/.test(importName))
                    continue;
                result.imports.push({
