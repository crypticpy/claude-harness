...
exports.formatSummaryAsText = formatSummaryAsText;
function generateSummary(parseResult, filePath) {
...
function calculateComplexity(result) {
    const score = result.functions.length * 2 +
...
function extractKeySymbols(result) {
    const symbols = [];
...
function inferPurpose(result, filePath) {
    const parts = [];
    ...
    const mainFunctions = result.functions.filter(f => /^(main|run|execute|start|init|setup|create|build|handle|process)$/i.test(f.name));
...
function formatSummaryAsText(summary) {
    ...
    const exportedNotInKey = summary.exports.filter(e => !summary.keySymbols.some(k => k.includes(e)));
