...
3. **No self-calls.** Hooks must not invoke Claude. Memory/diagnosis go through `llm-call.mjs` → OpenAI Responses API (`/v1/responses`). Two roles in `config.json` `llm`: `summarize` = `gpt-5-mini` (low effort) for the per-compaction session summary in `precompact-llm.mjs`; `recall` = `gpt-5.4-mini` (medium effort) for on-demand `recall_history` / deep-retrospective / self-evolution. Reasoning models: never send `temperature`, and `maxTokens` (→ `max_output_tokens`) covers reasoning + visible output. Per-edit LLM summarization is disabled (`rolling_log.backgroundEnrichment: false`) — the rolling log records edits locally with no LLM call. Routing Claude through its own hooks creates feedback loops.
